...
class PlayerInfoFrame(ttk.Frame):
    ...
    def set_info(self, player_info: PlayerInfo):
        ...
        if armor > 0:
            hp_status_info_string += (
                f' | Armor {"(with Helmet)" if has_helmet else ""}: {armor}'
            )
        ...
        match SideType.from_str(player_info["side"]):
            case SideType.T:
                ...
                if hp == 0:
                    self.hp_bar_fill_color = "red"
                    self.info_label.insert(tk.END, "Player is dead.\n")
                    self.health_bar_canvas.create_rectangle(
                        0, 0, self.health_bar_canvas.winfo_width(), 5, fill="red"
                    )
            case SideType.CT:
        ...
        self.health_bar_canvas.create_rectangle(
            0, 0, hp_bar_width, 5, fill=hp_bar_fill_color
        )
...
class HeatmapMenuButtonNames(Enum):
    ...
    GENERATE_ROUTINES_HEATMAP_FROM_DIRECTORY = (
        "Generate Routines Heatmap from Directory"
    )

    VIEW_ROUTINE_HEATMAP_COMPOSITION_INFO = (
        "View List of Demos Used in Heatmap Creation"
    )
